...
use PluginPresalesQuotationLine;
use Session;
use State;
use UserTitle;
...
class AddElementsView extends CommonGLPIView
{
    ...
    public function showTitle()
    {
        ...
        echo __("Add elements form", "manageentities");
        echo "</div>
               </h3>";
    }
    ...
    public function showFormEntity()
    {
        ...
        if (isset($this->pModel->getEntity()->fields['name'])
            && ($this->pModel->getEntity()->fields['name'] ?? '') != ""
    ...
    public function initJSPresalesContract($contractContent)
    {
        ...
        $this->showJSfunction(
            "addOnlyContract" . $contractContent['params']['rand'],
            $contractContent['idDivAjax'],
            $pModel->getUrl(),
            $contractContent['listIds'],
            $contractContent['params']
        );
        $this->initFormAddPDFContract();
    }
    ...
    public function initJSPresalesIntervention($interventionContent)
    {
        ...
        if (is_array($interventionContent)) {
            ...
            foreach ($interventionContent as $intervention) {
                $this->showJSfunction(
                    "addOnlyIntervention" . $i,
                    $intervention['idDivAjax'],
                    $pModel->getUrl(),
                    $intervention['listIds'],
                    $intervention['params'],
                    $intervention['idDivStakeholdersAjax']
                );
                $this->showJSfunction(
                    "addAnotherIntervention" . $i,
                    $intervention['idDivNewIntervention'],
                    $pModel->getUrl(),
                    $intervention['listIds'],
                    $intervention['paramsAddNewIntervention']
                );
                $i++;
            }
        }
    ...
    private function initJSFunctions($entityContent, $contactContent, $contractContent, $interventionContent)
    {
        ...
        $this->showJSfunction(
            "addOnlyEntity",
            $entityContent['idDivAjax'],
            $pModel->getUrl(),
            $entityContent['listIds'],
            $entityContent['params']
        );
        ...
        if (is_array($contactContent)) {
            ...
            foreach ($contactContent as $content) {
                ...
                $this->showJSfunction(
                    "addAnotherContact" . $i,
                    $content['idDivNewContact'],
                    $pModel->getUrl(),
                    $content['listIds'],
                    $content['paramsAddNewContact']
                );
                $i++;
            }
        }
        ...
        $this->showJSfunction(
            "addEntityAndContact",
            $contactContent[1]['idDivAjax'],
            $pModel->getUrl(),
            $contactContent[1]['listIds'],
            $contactContent[1]['params']
        );
        ...
        $this->showJSfunction(
            ...
            $interventionContent[1]['idDivAjax'],
            $pModel->getUrl(),
            $interventionContent[1]['listIds'],
            $interventionContent[1]['params']
        );
    ...
    public function showFormAddEntity()
    {
        ...
        $this->updateImgTabTitle(
            "'img_" . $this->pModel->getEntity()->getType() . "1'",
            $this->pModel->getMessage(ElementType::ENTITY, Status::NOT_SAVED),
            true
        );
        echo "\" >";
        ...
        Html::textarea([
            'name' => 'entity_comment',
            'value' => $currentEntity->fields["comment"],
        ...
        echo Html::input(
            'entity_phonenumber',
            ['value' => $currentEntity->fields["phonenumber"], 'id' => 'entity_phonenumber']
        );
        echo "</td>";
        ...
        Html::textarea([
            'name' => 'entity_address',
            'value' => $currentEntity->fields["address"],
        ...
        echo "<td>" . __('Fax') . "</td>";
        echo "<td>";
        echo Html::input('entity_fax', ['value' => $currentEntity->fields["fax"], 'id' => 'entity_fax']);
        echo "</td></tr>";
        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Website') . "</td>";
        echo "<td>";
        echo Html::input('entity_website', ['value' => $currentEntity->fields["website"], 'id' => 'entity_website']);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . _n('Email', 'Emails', 1) . "</td>";
        echo "<td>";
        echo Html::input('entity_email', ['value' => $currentEntity->fields["email"], 'id' => 'entity_email']);
        echo "</td></tr>";
        ...
        echo Html::input(
            'entity_postcode',
            ['value' => $currentEntity->fields["postcode"], 'id' => 'entity_postcode', 'size' => 7]
        );
        echo "</td></tr>";
        ...
        echo "<td>";
        echo Html::input('entity_state', ['value' => $currentEntity->fields["state"], 'id' => 'entity_state']);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Country') . "</td>";
        echo "<td>";
        echo Html::input('entity_country', ['value' => $currentEntity->fields["country"], 'id' => 'entity_country']);
        echo "</td></tr>";


        echo "<tr class='tab_bg_2'>";
        echo "<td colspan='4' class='right'>";
        echo "<input form='' type='submit' class='submit btn btn-primary manageentities_button' name='btnAddEntity' id='btnAddEntity' ";
    ...
    public function showFormAddContact($idContact = 0)
    {
        ...
        $this->updateImgTabTitle(
            "'img_" . $currentContact->getType() . ($idContact) . "'",
            $this->pModel->getMessage(ElementType::CONTACT, Status::NOT_SAVED),
            true
        );
        echo "\" >";
        ...
        $name = "fakeid_new_contact" . $rand;
        echo Html::hidden($name, ['id' => $name, 'value' => $idContact]);
        ...
        echo "<label for='previous_entity_for_contact" . $rand . "'>";
        ...
        echo "<td>";
        $name = "contact_name" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['name'] ?? ''), 'id' => $name]);
        echo "</td>";

        echo "<td rowspan='4' class='left'>" . __('Comments') . "</td>";
        echo "<td class='left' rowspan='4'>";
        $name = "contact_comment" . $rand;
        Html::textarea([
            'name' => $name,
            'value' => $currentContact->fields["comment"],
            'editor_id' => $name,
            'enable_richtext' => false,
            'cols' => 45,
            'rows' => 7,
        ...
        echo "<td>";
        $name = "contact_firstname" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['firstname'] ?? ''), 'id' => $name]);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Phone') . "</td>";
        echo "<td>";
        $name = "contact_phone" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['phone'] ?? ''), 'id' => $name]);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Phone 2') . "</td>";
        echo "<td>";
        $name = "contact_phone2" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['phone2'] ?? ''), 'id' => $name]);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Mobile phone') . "</td>";
        echo "<td>";
        $name = "contact_mobile" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['mobile'] ?? ''), 'id' => $name]);
        echo "</td>";
        echo "<td class='middle'>" . __('Address') . "</td>";
        echo "<td class='middle'>";
        $name = "contact_address" . $rand;
        Html::textarea([
            'name' => $name,
            'value' => $currentContact->fields["address"],
            'editor_id' => $name,
            'enable_richtext' => false,
            'cols' => 37,
            'rows' => 3,
        ...
        echo "<td>" . __('Fax') . "</td>";
        echo "<td>";
        $name = "contact_fax" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['fax'] ?? ''), 'id' => $name]);
        echo "</td>";
        echo "<td>" . __('Postal code') . "</td>";
        echo "<td>";
        $name = "contact_postcode" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['postcode'] ?? ''), 'id' => $name, 'size' => 7]);
        echo "</td></tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<td>" . _n('Email', 'Emails', 1) . "</td>";
        echo "<td>";
        $name = "contact_email" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['email'] ?? ''), 'id' => $name]);
        echo "</td>";

        echo "<td>" . __('City') . "</td>";
        echo "<td>";
        $name = "contact_town" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['town'] ?? ''), 'id' => $name, 'size' => 27]);
        echo "</td></tr>";
        ...
        echo "<td>" . _x('location', 'State') . "</td>";
        echo "<td>";
        $name = "contact_state" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['state'] ?? ''), 'id' => $name]);
        echo "</td></tr>";
        ...
        echo "<td>" . __('Country') . "</td>";
        echo "<td>";
        $name = "contact_country" . $rand;
        echo Html::input($name, ['value' => ($currentContact->fields['country'] ?? ''), 'id' => $name]);
        echo "</td></tr>";
        ...
        echo "<tr class='tab_bg_2'>";
        echo "<td colspan='4' class='right'>";
        echo "<input form='' type='submit' class='submit btn btn-primary manageentities_button' name='btnAddContact" . $idContact . "' id='btnAddContact" . $idContact . "' ";
    ...
    public function showFormAddContract($params = [])
    {
        ...
        echo "   <div id='tabs-1' style='padding:0px;' onchange=\"javascript:";
        $this->updateImgTabTitle(
            "'img_" . $this->pModel->getContract()->getType() . "1'",
            $this->pModel->getMessage(ElementType::CONTRACT, Status::NOT_SAVED),
            true
        );
        echo "\" >";
        ...
        if (!isset($params["presales"]) || (isset($params["presales"]) && $params["presales"] == false)) {
            ...
            echo "<label for='previous_entity_for_contract'>";
        ...
        echo "<td>";
        $name = "contract_num";
        echo Html::input($name, ['value' => ($currentContract->fields['num'] ?? ''), 'id' => $name]);
        echo "</td>";
        ...
        $idDpContractDuration = \Dropdown::showNumber("contract_duration", [
            'value' => $currentContract->fields["duration"],
            'min' => 1,
            'max' => 120,
        ...
        $name = "contract_accounting_number";
        echo Html::input($name, ['value' => ($currentContact->fields['accounting_number'] ?? ''), 'id' => $name]);
        echo "</td></tr>";
        echo "<tr class='tab_bg_1'>";
        echo "<td>" . __('Contract renewal period') . "</td><td>";
        $idDpInterventionicity = \Dropdown::showNumber("contract_periodicity", [
            'value' => $currentContract->fields["periodicity"],
            'min' => 2,
            'max' => 60,
            'step' => 12,
            'unit' => 'month',
        ...
        echo "<td>";
        $idDpBilling = \Dropdown::showNumber("contract_billing", [
            'value' => $currentContract->fields["billing"],
            'min' => 12,
            'max' => 60,
            'step' => 12,
            'unit' => 'month',
        ...
        $idMaxItems = \Dropdown::showNumber("contract_max_links_allowed", [
            'value' => $currentContract->fields["max_links_allowed"],
            'min' => 1,
            'max' => 200,
            "rand" => $rand,
        ...
        Html::textarea([
            'name' => $name,
            'value' => $currentContract->fields["comment"],
            'editor_id' => $name,
            'enable_richtext' => false,
            'cols' => 50,
            'rows' => 4,
        ...
        $idDpContractWkBeginHour = $this->showHours(
            "contract_week_begin_hour",
            $currentContract->fields["week_begin_hour"],
            0,
            $rand
        );

        echo "<span class='small_space'>" . __('End') . "</span>&nbsp;";
        $idDpContractWkEndHour = $this->showHours(
            "contract_week_end_hour",
            $currentContract->fields["week_end_hour"],
            0,
            $rand
        );
        echo "</td></tr>";
        ...
        echo "<td colspan='3'>";
        $idDpYNsat = \Dropdown::showYesNo(
            "contract_use_saturday",
            $currentContract->fields["use_saturday"],
            -1,
            ["rand" => $rand]
        );
        echo "<span class='small_space'>" . __('Start') . "</span>&nbsp;";
        $idDpContractSatBeginHour = $this->showHours(
            "contract_saturday_begin_hour",
            $currentContract->fields["saturday_begin_hour"],
            0,
            $rand
        );
        echo "<span class='small_space'>" . __('End') . "</span>&nbsp;";
        $idDpContractSatEndHour = $this->showHours(
            "contract_saturday_end_hour",
            $currentContract->fields["saturday_end_hour"],
            0,
            $rand
        );
        echo "</td></tr>";
        ...
        echo "<td colspan='3'>";
        $idDpYNmon = \Dropdown::showYesNo(
            "contract_use_sunday",
            $currentContract->fields["use_sunday"],
            -1,
            ["rand" => $rand]
        );
        echo "<span class='small_space'>" . __('Start') . "</span>&nbsp;";
        $idDpContractSunBeginHour = $this->showHours(
            "contract_sunday_begin_hour",
            $currentContract->fields["sunday_begin_hour"],
            0,
            $rand
        );
        echo "<span class='small_space'>" . __('End') . "</span>&nbsp;";
        $idDpContractSunEndHour = $this->showHours(
            "contract_sunday_end_hour",
            $currentContract->fields["sunday_end_hour"],
            0,
            $rand
        );
        echo "</td></tr>";
        ...
        echo "<tr  class='tab_bg_2'>";
        echo "<td colspan='4' class='right'>";
        echo "<input form='' type='submit' class='submit btn btn-primary manageentities_button' name='btnAddContract' id='btnAddContract' value='";
        if (isset($currentContract->fields['id']) && $currentContract->fields['id'] ?? '' > 0) {
        ...
        $listId = [
            ...
            "dropdown_contract_use_sunday" . $idDpYNmon => ["dropdown", "new_contract_use_sunday"],
            ...
            "contract_sunday_begin_hour" . $idDpContractSunBeginHour => ["dropdown", "new_contract_sun_begin_hour"],
            "contract_sunday_end_hour" . $idDpContractSunEndHour => ["dropdown", "new_contract_sun_end_hour"],
    ...
    public function showListPDFcontract($display = true)
    {
        ...
        $query = $arr['query'] ?? null;
        $linkparam = $arr['linkparam'] ?? '';
    ...
    private function initFormAddPDFContract()
    {
        ...
        echo "<td>";
        $contract = $this->pModel->getContract();
        $entitiesId = $contract->fields['entities_id'] ?? '';
    ...
    public function showFormAddContractManagementType(\Contract $contract)
    {
        ...
        echo "<td>";
        $name = "cmanagetype_date_signature" . $idPluginContract;
        echo Html::input($name, [
            'value' => (isset($pluginContract['date_signature'])
        ...
        $name = "cmanagetype_date_renewal" . $idPluginContract;
        echo Html::input($name, [
            'value' => (isset($pluginContract['date_renewal'])
        ...
        echo "<td>";
        $idDpYNonGANT = \Dropdown::showYesNo(
        ...
        echo "<td>";
        Html::showCheckbox([
            'name' => 'refacturable_costs',
            'id' => 'refacturable_costs',
            'checked' => $pluginContract["refacturable_costs"] ?? 1,
        ]);
        echo "</td></tr>";
        ...
        $rand = \Dropdown::showYesNo(
            "moving_management",
            $pluginContract["moving_management"] ?? 0,
            -1,
      ...
      "
        ...
        $rand_duration = \Dropdown::showTimeStamp(
            'duration_moving',
            [
        ...
        if ($canEdit) {
            ...
            echo "<input form='' type='submit' class='submit btn btn-primary manageentities_button' name='btnAddContractManagementType' id='btnAddContractManagementType'";
    ...
    public function showFormAddInterventions($idIntervention = 1, $paramsF = [])
    {
        ...
        $unit = Contract::getUnitContractType($config, $pluginContract['contract_type'] ?? []);


        // Onchange for img purpose
        echo "   <div id='tabs-" . ($idIntervention) . "' style='padding:0px;' onchange=\"javascript:";
        $this->updateImgTabTitle(
            "'img_" . $currentContractday->getType() . ($idIntervention) . "'",
            $this->pModel->getMessage(ElementType::INTERVENTION, Status::NOT_SAVED),
            true
        );
        echo "\" >";

        $name = "fakeid_new_intervention" . $rand;
        echo Html::hidden($name, ['id' => $name, 'value' => $idIntervention]);
        ...
        if (!isset($paramsF["presales"])) {
            ...
            $entity = $this->pModel->getEntity();
            $entityId = $entity->fields['id'] ?? '';
            $currentContractdayEntitiesId = $currentContractday->fields['entities_id'] ?? '';
            $cond = false;
            if (isset($currentContractday->fields['entities_id']) && isset($entity->fields['id']) && $entityId > 0 && $currentContractday->fields['entities_id'] == $entityId) {
                $cond = true;
            } elseif ((!isset($currentContractday->fields['entities_id']) || $currentContractday->fields['entities_id'] == "") && $entityId !== '' && $entityId > 0) {
                $cond = true;
            }
            if ($cond) {
                echo " style='visibility:hidden;' ";
            ...
            echo "<label for='previous_entity_for_intervention" . $rand . "'>";
            ...
            echo "<label for='previous_contract_for_intervention" . $rand . "'>";
            ...
            echo "&nbsp;" . __("New contract created previously", "manageentities") . "</label>";
            echo "</td>";
        } else {
            $name = "presales" . $rand;
            echo Html::hidden($name, ['id' => $name, 'value' => $paramsF["presales"]]);
            echo "<td hidden>";
        ...
        if ($config->fields['hourorday'] == Config::DAY) {
            ...
            $idDpContractType = Contract::dropdownContractType(
                "contract_type",
                $currentContractday->fields['contract_type'] ?? ''
            );
            echo "</td>";
        } else {
            echo "<td colspan='2'></td>";
        ...
        echo "<td>";
        $name = "intervention_name" . $idIntervention;
        echo Html::input($name, ['value' => ($currentContractday->fields['name'] ?? ''), 'id' => $name]);
        echo "</td>";

        if ($config->fields['hourorday'] == Config::HOUR
            && isset($pluginContract['contract_type']) && $pluginContract['contract_type'] == Contract::CONTRACT_TYPE_UNLIMITED) {
            echo "<td></td><td></td>";
        } else {
            echo "<td>" . __('Postponement', 'manageentities') . "</td>";
            echo "<td>";
            $name = "report" . $idIntervention;
            echo Html::input(
                $name,
                [
        ...
        echo "<td>";
        $name = "intervention_begin_date" . $idIntervention;
        echo Html::input($name, [
            'value' => (isset($currentContractday->fields['begin_date'])
            && $currentContractday->fields['begin_date'] ?? '' != "NULL"
            && $currentContractday->fields['begin_date'] ?? '' ? Html::convDate(
        ...
        $name = "intervention_end_date" . $idIntervention;
        echo Html::input($name, [
            'value' => (isset($currentContractday->fields['end_date'])
        ...
        //      $resultCriDetail = CriDetail::getCriDetailData($currentContractday->fields);
        //      foreach($resultCriDetail['result'] as $dataCriDetail){
        //         //Conso
        //         $conso += $dataCriDetail['conso'];
        //      }
        //      var_dump($this->pModel->getCriPrice($idIntervention));
        //      if ($config->fields['useprice'] == Config::PRICE && ($this->pModel->getCriPrice($idIntervention) == null || (is_array($this->pModel->getCriPrice($idIntervention)) && sizeof($this->pModel->getCriPrice($idIntervention)) <= 0 ))) {
      ...
      onclick=\"javascript:addAnotherIntervention" . $idIntervention . "();\"/>";
        ...
        $params = [
            'action' => Action::UPDATE_CONTRACT_LIST,
        ...
        $this->showJSfunction(
            "updateContractList" . $realRand,
            "div_select_contract_for_intervention" . $idIntervention,
            $this->pModel->getUrl(),
            $listId,
            $params
        );
    ...
    public function initCriPricesView($item, $fakeIdIntervention, $idCriPrice = -1)
    {
      ...
      ";
        ...
        if ($canedit) {
            ...
            $params = [
                'action' => Action::SHOW_FORM_CRI_PRICE,
            ...
            $this->showJSfunction(
                "showFormCriPrice" . $rand,
                "viewcriprice_" . $fakeIdIntervention,
                $this->pModel->getUrl(),
                [],
                $params
            );
    ...
    public function updateListItems($item, $fakeIdIntervention, $criPrice, $status)
    {
        ...
        switch ($status) {
            case Status::ADDED:
                ...
                $this->showJSfunction(
                    "showFormCriPrice" . $criPrice->fields['id'] ?? '',
                    "viewcriprice_" . $fakeIdIntervention,
                    $this->pModel->getUrl(),
                    $listId,
                    $params
               ...
               document.getElementById('tablelistcriprices" . $fakeIdIntervention . "').style.visibility='visible';
                ...
                $this->addRowOnTable(
                    'tablelistcriprices' . $fakeIdIntervention,
                    'row_criprice_' . $criPrice->fields['id'] ?? '',
                    $cells,
                    null
                );
                break;
            ...
            case Status::UPDATED:
                ...
                $this->updateCellOnTable(
                    'price_' . $criPrice->fields['id'] ?? '',
                    Html::formatNumber($criPrice->fields['price'] ?? '', true)
                );
                $this->updateCellOnTable(
                    'is_default_' . $criPrice->fields['id'] ?? '',
                    \Dropdown::getYesNo($criPrice->fields['is_default'] ?? '')
                );
    ...
    public function listCriPrices($ID, $data, $canedit, $rand, $idCriPrice, $fakeIdIntervention)
    {
        ...
        foreach ($data as $field) {
            ...
            if ($canedit) {
                ...
                $this->showJSfunction(
                    "showFormCriPrice" . $field['id'] . $rand,
                    "viewcriprice_" . $fakeIdIntervention,
                    $this->pModel->getUrl(),
                    $listId,
                    $params
    ...
    public function showFormCriPrice($idCriPrice, $idIntervention, $fakeIdIntervention, $options)
    {
        ...
        // Cri Type
        echo "<td>";
        echo CriType::getTypeName() . '&nbsp;' . $this->pModel->getMessage("mandatory_field");
        $name = "id_criprice" . $item->fields['id'] ?? '';
        echo Html::hidden($name, ['id' => $name, 'value' => $item->fields['id'] ?? '']);

        echo "</td>";

        echo "<td>";
        $idDpCriType = \Dropdown::show(CriType::class, [
        ...
        echo "<td>";
        $value = 1;
        if (isset($item->fields['is_default']) && !empty($item->fields['is_default'] ?? '')) {
            $value = $item->fields['is_default'] ?? '';
        }
        $idDpIsdefault = \Dropdown::showYesNo('is_default', $value);
        echo "</td>";
        echo "<td colspan='2'></td>";
        echo "</tr>";
        ...
        echo "<input type='button' class='submit btn btn-primary manageentities_button' name='btn_add_cprice_" . $rand . "' id='btn_add_cprice_" . $rand . "'";
        ...
        if (isset($item->fields['id']) && $item->fields['id'] ?? '' > 0) {
            echo "<input type='button' class='submit btn btn-primary manageentities_button' style='margin-left:16px;' name='btn_delete_cprice_" . $rand . "' id='btn_delete_cprice_" . $rand . "'";
        ...
        // Variables to ajax add entity
        $listId = [
            "dropdown_is_default" . $idDpIsdefault => ["dropdown", "new_criprice_is_default"],
        ...
        $params = [
            'action' => Action::UPDATE_CRI_PRICE_FROM_TYPE,
            "id_div_ajax" => "viewcriprice_" . $fakeIdIntervention,
            "plugin_manageentities_contractdays_id" => $parent->getField('id'),
            "entities_id" => $parent->getField('entities_id'),
            "parent" => ContractDay::class,
    ...
    public function updateCriPriceFromType($price, $post)
    {
        if ($price == null && $post == null) {
            ...
            if ($post['new_criprice_critype'] <= 0) {
                ...
                if (null == $price) {
                    $this->changeBtnName(
                        "btn_add_cprice_" . $post['fakeid_new_intervention'],
                        __("Add this price", "manageentities")
                    );
    ...
    private function showBtnRAZ()
    {
        ...
        $target = PLUGIN_MANAGEENTITIES_WEBDIR . "/ajax/addelements.listener.php";
    ...
    private function showBtnAddAll()
    {
        ...
        echo "<input form='' type='submit' class='submit btn btn-primary manageentities_button' name='btnAddAll' id='btnAddAll' ";
        ...
        switch ($typeField) {
            case ElementType::ENTITY:
                if (isset($_POST['new_entity_name']) && $_POST['new_entity_name'] != "") {
                    ...
                    $this->showMessage(
                        $pModel->getMessage(ElementType::ENTITY, Errors::ERROR_FIELDS),
            ...
            case ElementType::CONTACT:
                if (isset($_POST['new_contact_name']) && $_POST['new_contact_name'] != "") {
                    ...
                    $this->showMessage(
                        $pModel->getMessage(ElementType::CONTACT, Errors::ERROR_FIELDS),
                        Messages::MESSAGE_ERROR,
                        -1,
                        -1,
                        "contact_name" . $_POST['fakeid_new_contact']
                    );
            ...
            case ElementType::CONTRACT:
                if (isset($_POST['new_contract_name']) && $_POST['new_contract_name'] != "") {
                    ...
                    $this->showMessage(
                        $pModel->getMessage(ElementType::CONTRACT, Errors::ERROR_FIELDS),
                        Messages::MESSAGE_ERROR,
                        -1,
                        -1,
                        "contract_name"
            ...
            case ElementType::INTERVENTION:
                ...
                if (isset($_POST['new_intervention_name']) && $_POST['new_intervention_name'] != "") {
                    ...
                    if (isset($_POST['new_intervention_begin_date']) && $_POST['new_intervention_begin_date'] != "") {
                        $this->bordersOnError(
                            "intervention_begin_date" . $_POST['fakeid_new_intervention'],
                            false,
                            false
                        );
                        ...
                        if (isset($_POST['new_intervention_contractstate_id']) && $_POST['new_intervention_contractstate_id'] > 0) {
                            $this->bordersOnError(
                                "div_select_contractstate" . $_POST['fakeid_new_intervention'],
                                false,
                                false
                            );
                            ...
                            if ((isset($_POST['new_intervention_contract_id']) && $_POST['new_intervention_contract_id'] > 0)
                                ...
                                $this->showMessage(
                                    $pModel->getMessage(ElementType::INTERVENTION, Errors::ERROR_FIELDS),
                                    Messages::MESSAGE_ERROR,
                                    -1,
                                    -1,
                                    "cell_select_contract_for_intervention" . $_POST['fakeid_new_intervention']
                    ...
                    $this->showMessage(
                        ...
                        -1,
                        -1,
                        "intervention_name" . $_POST['fakeid_new_intervention']
                    );
                    $pModel->addError(
                        Errors::ERROR_INTERVENTION,
                        Errors::ERROR_ADD,
            ...
            case ElementType::CRIPRICE:
                if (isset($_POST['new_criprice_pricefield']) && $_POST['new_criprice_pricefield'] != "") {
                    ...
                    if (isset($_POST['new_criprice_critypes']) && $_POST['new_criprice_critypes'] > 0) {
                        $this->bordersOnError(
                            "dropdown_plugin_manageentities_critypes_id" . $_POST['fakeid_new_intervention'],
                            false,
                            false
                        );
                        return true;
                    ...
                    $this->showMessage(
                        ...
                        -1,
                        -1,
                        "textfield_price" . $_POST['fakeid_new_intervention']
                    );
                    $pModel->addError(Errors::ERROR_CRIPRICE, Errors::ERROR_ADD, 'true');
                }
                break;
        ...
        switch ($content['result']) {
            ...
            case Action::ADD_ONLY_ENTITY:
                ...
                $this->showAlertsJQ(
                    $_POST['id_div_ajax'],
                    $idToSend,
                    __("The previous entity will be added, continue ?", "manageentities")
                );
                break;
            case Action::ADD_ONLY_CONTRACT:
                $this->showAlertsJQ(
                    ...
                    __("The previous contract will be added, continue ?", "manageentities")
                );
                break;
            case Action::ADD_ENTITY_AND_CONTRACT:
                $this->showAlertsJQ(
                    $_POST['id_div_ajax'],
                    "entity-intervention-contract",
                    __("The previous entity and contract will be added, continue ?", "manageentities")
                );
                break;
            case Action::ADD_ALL_ELEMENT:
                $this->showAlertsJQ(
                    $_POST['id_div_ajax'],
                    "add-all-element",
                    __("All previous elements will be added, continue ?", "manageentities")
                );
                break;
            case Action::UPDATE_ALL_ELEMENT:
                $this->showAlertsJQ(
                    $_POST['id_div_ajax'],
                    "update-all-element",
                    __("All previous elements will be updated, continue ?", "manageentities")
                );
                break;
            case Action::DELETE_CONTRACT_MANAGEMENT_TYPE:
                $this->showAlertsJQ(
                    ...
                    $pModel->getMessage("irreversible_action")
                );
                break;
            default:
                break;
